...
if ( ! function_exists( 'newspanda_pagination_style' ) ) :
	/**
	 * Styles the pagination style.
	 *
	 * @see select_pagination_style().
	 */
	function newspanda_pagination_style() {
	$select_pagination_style = newspanda_get_option( 'select_pagination_style');
	...
	<div class="newspanda-nav-pagination">
		...
		newspanda_ajax_pagination( $select_pagination_style );

		elseif ( 'pagination_numeric' == $select_pagination_style ) :

			$prev_text        = sprintf(
				'<span class="nav-prev-text">%s</span>',
				newspanda_get_theme_svg('chevron-left')
			);
			$next_text        = sprintf(
				'<span class="nav-next-text">%s</span>',
				newspanda_get_theme_svg('chevron-right')
			);
			...
			$prev_text = sprintf(
				...
				__( 'Older Articles', 'newspanda' )
			);
			$next_text = sprintf(
				'%s <span class="nav-next-text title">%s</span>',
				'<span class="arrow" aria-hidden="true">&rarr;</span>',
				__( 'Newer Articles', 'newspanda' )
			);
