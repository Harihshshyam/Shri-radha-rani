 ...
 * @package NewsPanda
 */
if (!function_exists('newspanda_posted_on')) :
    /**
     ...
     */
    function newspanda_posted_on($date_format = null, $date_label = null, $date_meta = null)
    {
        ...
        <div class="entry-meta entry-date posted-on">
            ...
            if ($date_meta == 'with_icon') :
                newspanda_the_theme_svg('calendar');
            endif;
            ...
            if ($date_format == 'time_ago') {
                ...
                /* translators: %s: post date. */
                    esc_html_x('%s', 'post date', 'newspanda'),
...
if (!function_exists('newspanda_posted_by')) :
    /**
     * Prints HTML with meta information for the current post-date/time.
     ...
     */
    function newspanda_posted_by($author_meta = null, $meta_text = null)
    {
        // Author.
...
if (!function_exists('newspanda_post_category')) :
    /**
     ...
     * @since 1.0.0
     */
    function newspanda_post_category($show_color = null, $meta_text = null, $limit = 0)
    {
        ...
        <div class="entry-meta entry-categories cat-links<?php echo esc_attr($wrapper_class); ?>">
            ...
            if ($meta_text) : ?>
                <span class="entry-meta-label author-label"><?php echo esc_html($meta_text); ?></span>
            <?php else : ?>
                <span class="screen-reader-text"><?php esc_html_e('Posted in', 'newspanda'); ?></span>
            <?php endif;
            ?>
            <?php
            $style_attr = '';
...
if (!function_exists('newspanda_entry_footer')) :
    /**
     * Prints HTML with meta information for the categories, tags and comments.
     */
    function newspanda_entry_footer($show_tag = true, $show_comment = true)
    {
        // Hide category and tag text for pages.
        if ('post' === get_post_type() && $show_tag) {

            /* translators: used between list items, there is a space after the comma */
            $tags_list = get_the_tag_list('', esc_html_x(', ', 'list item separator', 'newspanda'));
            if ($tags_list) {
                /* translators: 1: list of tags. */
                printf('<span class="entry-meta entry-tags tags-links">' . esc_html__('Tags %1$s', 'newspanda') . '</span>', $tags_list); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            }
        }
            ...
            comments_popup_link(
                sprintf(
                    ...
                    /* translators: %s: post title */
                        __('Leave a Comment<span class="screen-reader-text"> on %s</span>', 'newspanda'),
                        array(
                            'span' => array(
                                'class' => array(),
                            ),
        ...
        edit_post_link(
            sprintf(
                ...
                /* translators: %s: Name of current post. Only visible to screen readers */
                    __('Edit <span class="screen-reader-text">%s</span>', 'newspanda'),
                    array(
                        'span' => array(
                            'class' => array(),
                        ),
...
if (!function_exists('newspanda_post_thumbnail')) :
    /**
     ...
     */
    function newspanda_post_thumbnail($image_class = null, $image_size = null)
    {
 ...
 *
 */
function newspanda_filter_wp_list_pages_item_classes($css_class, $page, $depth, $args)
{
...
add_filter('page_css_class', 'newspanda_filter_wp_list_pages_item_classes', 10, 4);
 ...
 *
 */
function newspanda_add_sub_toggles_to_main_menu($args, $item)
{
    ...
    if (isset($args->show_toggles) && $args->show_toggles) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
            $args->link_after = '<span class="icon">' . newspanda_get_theme_svg( 'chevron-down' ) . '</span>';
        } else {
            $args->link_after = '';
        }
	}
...
add_filter('nav_menu_item_args', 'newspanda_add_sub_toggles_to_main_menu', 10, 2);
 ...
 *
 */
function newspanda_nav_menu_social_icons($item_output, $item, $depth, $args)
{
    // Change SVG icon inside social links menu if there is supported URL.
    if ('social' === $args->theme_location) {
        $svg = NewsPanda_SVG_Icons::get_social_link_svg($item->url);
        if (empty($svg)) {
            $svg = newspanda_get_theme_svg('link');
        }
...
add_filter('walker_nav_menu_start_el', 'newspanda_nav_menu_social_icons', 10, 4);
 ...
 *
 */
function newspanda_toggle_duration()
{
    /**
     ...
     *
     */
    $duration = apply_filters('newspanda_toggle_duration', 250);

    return $duration;
}

if ( ! function_exists( 'newspanda_archive_post_count' ) ) {
    /**
     * Post Count in Archive Pages
     */
    function newspanda_archive_post_count() {
        global $wp_query;
        ...
        if ( $found_posts > 0 ) {
            ...
            <div class="wpi-archive-post-count">
                ...
                $found_posts_count = sprintf( _n( '%s post', '%s posts', $found_posts, 'newspanda' ), $found_posts );

                /**
                 * The newspanda_article_full_count hook.
                 *
                 * @since 1.0.0
                 */
                echo esc_html( apply_filters( 'newspanda_article_full_count', $found_posts_count, $found_posts ) );
                ?>
            </div>
            <?php
        }
...
if ( ! function_exists( 'newspanda_get_the_archive_title' ) ) {
    /**
     * Archive Title
     *
     ...
     */
    function newspanda_get_the_archive_title( $title ) {
        if ( is_category() ) {
...
add_filter( 'get_the_archive_title', 'newspanda_get_the_archive_title' );
