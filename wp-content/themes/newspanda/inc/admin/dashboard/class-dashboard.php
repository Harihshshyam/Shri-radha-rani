...
class NewsPanda_Dashboard
{
     ...
     * @since 1.0.0
     */
    private $preview_url = 'https://live-preview.wpinterface.com/newspanda/';
    /**
     * Misc Support Url
     *
     * @access private
     * @since 1.0.0
     */
    private $community_url = 'https://wordpress.org/support/theme/newspanda/';
    ...
    public function create_menu()
    {
        ...
        $theme_page_name = sprintf(esc_html__('%s', 'newspanda'), $theme->Name);
        add_theme_page(
            $theme_page_name,
            $theme_page_name,
            'edit_theme_options',
    ...
    {
        // Bail if we're not on a NewsPanda screen or page.
        if (empty($_REQUEST['page']) || false === strpos(sanitize_text_field(wp_unslash($_REQUEST['page'])), 'newspanda')) { // phpcs:ignore WordPress.Security.NonceVerification
            return;
        }
        global $wp_filter;
        $ignore_notices = apply_filters('newspanda_ignore_hide_admin_notices', array());
    ...
    public function dashboard_page()
    {
        ...
        $admin_url = admin_url();
        $tabs = apply_filters(
            'newspanda_dashboard_tabs',
            array(
                'dashboard' => array(
                    'name' => esc_html__('Dashboard', 'newspanda'),
                    'callback' => function () {
                        include __DIR__ . '/views/dashboard.php';
                    },
                ),
                'starter-templates' => array(
                    'name' => esc_html__('Starter Templates', 'newspanda'),
                    'callback' => function () {
                            include __DIR__ . '/views/starter-templates.php';
                    },
                ),
                'products' => array(
                    'name' => esc_html__('Our Recommendation', 'newspanda'),
                    'callback' => function () {
                        include __DIR__ . '/views/products.php';
                    },
                ),
                'free-vs-pro' => array(
                    'name' => esc_html__('Free Vs Pro', 'newspanda'),
                    'callback' => function () {
                        include __DIR__ . '/views/free-vs-pro.php';
                    },
                ),
                'help' => array(
                    'name' => esc_html__('Help', 'newspanda'),
                    'callback' => function () {
        ...
        ?>
        <div class="wrap">
            <div class="newspanda-dashboard-area">
                <div class="newspanda-dashboard-header">
                    <a class="newspanda-logo" href="<?php echo esc_url($this->theme_url); ?>"
                       target="_blank">
                        <img class="newspanda-premium-badge"
                             src="<?php echo esc_url(NEWSPANDA_PARENT_URL . '/inc/admin/dashboard/images/newspanda-logo.webp'); ?>"
                             alt="<?php esc_attr_e('NewsPanda', 'newspanda'); ?>">
                        <span class="newspanda-theme-version">
									<?php
                            ...
                            foreach ($tabs as $id => $tab) :
                                ...
                                <li class="menu-item menu-item-<?php echo esc_attr($id); ?>">
                                    <a id="<?php echo esc_attr($id); ?>"
                                       href=<?php echo esc_url("{$admin_url}themes.php?page=newspanda&tab=$id"); ?>><?php echo esc_html($tab['name']); ?></a>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    </nav>
                    <div class="dashboard-header-extras">
                        <div class="dashboard-button-group">
                            ...
                            <button type="button" id="dashboard-changelog-button" class="button button-secondary dashboard-button dashboard-button-secondary">
                                <?php esc_html_e('Changelog', 'newspanda'); ?>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                ?>
            </div><!--/.newspanda-dashboard-area-->
        </div><!--/.wrap-->
        <div id="newspanda-dashboard-offcanvas" class="newspanda-dashboard-offcanvas">
            <div class="dialog-head">
                <h3><?php esc_html_e('Latest Updates', 'newspanda'); ?></h3>
            ...
            <div class="dialog-content">
                <?php
                $changelog = new NewsPanda_Changelog_Parser();
                // Fetch changelog data.
                $changelog_data = $changelog->get_items();

                if ($changelog_data) {
                    // Output the changelog data
                    echo '<div class="newspanda-changelog">';
                    foreach ($changelog_data as $entry) {
                        echo '<div class="newspanda-changelog-list-item">';
                        echo '<div class="newspanda-changelog-list-head">';
                        echo '<h4 class="newspanda-changelog-version">Version: ' . esc_html($entry['version']) . '</h4>';
                        echo '<p class="newspanda-changelog-date">' . esc_html($entry['date']) . '</p>';
                        echo '</div>'; // newspanda-changelog-list-head
                        // Display each change
                        echo '<div class="newspanda-changelog-change">';
                        foreach ($entry['changes'] as $tag => $changes) {
                            echo '<div class="newspanda-changelog-change-item item--' . esc_html(strtolower($tag)) . '">';
                            echo '<span class="newspanda-changelog-change-type">' . esc_html($tag) . '</span>';
                            echo '<div class="newspanda-changelog-change-list">';
                            foreach ($changes as $change) {
                                echo '<p class="newspanda-changelog-change-desc">' . esc_html($change) . '</p>';
                            }
                            echo '</div>'; // newspanda-changelog-change-list
                            echo '</div >'; // newspanda-changelog-change-item
                        }
                        echo '</div>'; // newspanda-changelog-change
                        echo '</div>'; // newspanda-changelog-list-item
                    }
                    echo '</div>'; // newspanda-changelog
                } else {
