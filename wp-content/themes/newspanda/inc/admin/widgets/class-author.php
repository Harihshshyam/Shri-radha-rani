...
/**
 * Adds NewsPanda_Author_Widget widget.
 */
class NewsPanda_Author_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'newspanda_author_widget',
            esc_html__( 'NewsPanda: Author Widget', 'newspanda' ),
            array( 'description' => esc_html__( 'Displays author information.', 'newspanda' ) )
        );

        $this->social_networks = apply_filters(
            'newspanda_author_widget_social_networks',
            array(
                'facebook',
                'twitter',
                'linkedin',
    ...
    private function display_social_links( $instance ) {
        ...
        foreach ( $social_networks as $network ) {
            if ( ! empty( $instance[ $network ] ) ) {
                $svg = NewsPanda_SVG_Icons::get_social_link_svg( $instance[ $network ] );
                if ( $svg ) {
    ...
    public function form( $instance ) {
        $fields = array(
            'title'               => esc_html__( 'Title:', 'newspanda' ),
            'author_name'         => esc_html__( 'Author Name:', 'newspanda' ),
            'author_pos'          => esc_html__( 'Author Position:', 'newspanda' ),
            'author_desc'         => esc_html__( 'Description:', 'newspanda' ),
            'author_img'          => esc_html__( 'Image:', 'newspanda' ),
            'image_border_radius' => esc_html__( 'Enable Image Border Radius', 'newspanda' ),
            'social_border_radius' => esc_html__( 'Circular Social Icon', 'newspanda' ),
            'social_links_color'  => esc_html__( 'Social Icon Color', 'newspanda' ),
        );
            ...
            case 'author_img': ?>
                ...
                <div class="image-field">
                    ...
                    <p>
                        ...
                        <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'newspanda' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'newspanda' ); ?>">
                            <?php esc_html_e( 'Upload/Add image', 'newspanda' ); ?>
                        </button>
                        <button type="button" class="remove_image_button button" style="<?php echo esc_attr( $remove_button_style ); ?>"><?php esc_html_e( 'Remove image', 'newspanda' ); ?></button>
                    </p>
                </div>
    ...
    private function get_select_options( $field ) {
        switch ( $field ) {
            case 'social_links_color':
                return array(
                    'has-brand-background'  => esc_html__( 'Has Brand Background', 'newspanda' ),
                    'has-default-color'   => esc_html__( 'Theme Color', 'newspanda' ),
                    'has-brand-color'  => esc_html__( 'Has Brand Color', 'newspanda' ),
                );
