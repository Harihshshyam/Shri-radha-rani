 ...
 * @package NewsPanda
 */
$args = newspanda_get_archive_header_query_args();
$enable_archive_featured_post = newspanda_get_option('enable_archive_featured_post');
$enable_archive_post_count = newspanda_get_option('enable_archive_post_count');
?>
<header class="page-header archive-header">
    <?php
    newspanda_get_breadcrumb();
    the_archive_title('<h1 class="page-title">', '</h1>');
    the_archive_description('<div class="archive-description">', '</div>');
    if ($enable_archive_post_count) {
        newspanda_archive_post_count();
    }
    ?>
</header><!-- .page-header -->
        ...
        <div class="archive-header-wrapper">
            ...
            if ($query_archive_header->have_posts()) {
                while ($query_archive_header->have_posts()) {
                    ...
                    $newspanda_single_post_featured = get_post_meta($post->ID, 'newspanda_single_post_featured_post', true);
                    if($newspanda_single_post_featured){
                        ++$counter;
                        if (1 === $counter) {
                            $excerpt = 'true';
                            ?>
                            <article id="archive-image-<?php the_ID(); ?>" <?php post_class('wpi-post wpi-post-default'); ?>>
                                ...
                                <div class="entry-details">
                                    ...
                                    if ($enable_category_meta) {
                                        newspanda_post_category($select_category_color, $archive_category_label);
                                    }
                                    ?>
